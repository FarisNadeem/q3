...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // Get a list of existing persons
        List<Person> persons = PersonManager.getPersons();

        // Create a person with your name and surname
        Person myself = new Person();
        myself.setId("BR9451354");
        myself.setName("Faris");
        myself.setSurname("Nadeem");

        // Choose a room for check-in (for simplicity, let's assume room number 101)
        Room room = new Room(102);

        // Check-in yourself and one of the persons from the list into the same room
        hotelManager.checkIn(myself, room);
        hotelManager.checkIn(persons.get(1), room);
    ...
    static class HotelManager {
        ...
        public void checkIn(Person person, Room room) {
            ...
            System.out.println(person.getName() + " " + person.getSurname() + " checked in to room number " + room.getRoomNumber());
        }

        public void checkOut(Person person) {
            guestRooms.remove(person.getId());
            System.out.println(person.getName() + " " + person.getSurname() + " checked out.");
        }
        ...
        public void displayInfo() {
            ...
            for (Map.Entry<String, Room> entry : guestRooms.entrySet()) {
                ...
                if (person != null) {
                    System.out.println("Guest: " + person.getName() + " " + person.getSurname() + ", Room: " + entry.getValue().getRoomNumber());
                }
            }
        }

        private Person findPersonById(String id, List<Person> persons) {
